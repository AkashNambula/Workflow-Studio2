...
async def async_email_dispatch(
...
async def run_workflow(
    ...
    try:
        ...
        while current_node_id:
            print(f"[SAGA] Current Node = {current_node_id}")
            ...
            print(f"[SAGA] Executing Node Type = {node_type_normalized}")

            try:
                await broadcast_node_state(
                    run_id,
                    current_node_id,
                    "RUNNING"
                )
                print("[SAGA] Broadcast Done")
            except Exception as e:
                print(f"[SAGA] Broadcast Error: {e}")

            for employee in employees:

                emp_name = (
                    str(getattr(employee, "name", ""))
                    if hasattr(employee, "name")
                    else employee.get("name", "")
                )

                emp_email = (
                    str(getattr(employee, "email", ""))
                    if hasattr(employee, "email")
                    else employee.get("email", "")
                )

                emp_phone = (
                    str(getattr(employee, "phone", ""))
                    if hasattr(employee, "phone")
                    else employee.get("phone", "")
                )

                emp_role = (
                    str(getattr(employee, "role", ""))
                    if hasattr(employee, "role")
                    else employee.get("role", "")
                )

                emp_date = (
                    str(getattr(employee, "joining_date", ""))
                    if hasattr(employee, "joining_date")
                    else employee.get("joining_date", "")
                )
                ...
                if "condition" in node_type_normalized:
                    ...
                    condition_result = ConditionNode().execute(
                        ...
                        joining_date=emp_date
                    )

                    if condition_result:

                        logs.append(
                            f"🟢 Condition SUCCESS validation passed for {emp_name}."
                        )
                        ...
                        current_node_id = (
                            ...
                            if next_edge and isinstance(next_edge, dict)
                            else (
                                getattr(next_edge, "target", None)
                                if next_edge
                                else None
                            )
                        )
                        ...
                        logs.append(
                            f"🔴 Condition FAILED validation for {emp_name}."
                        )
                    ...
                    logs.append(
                        f"Document generation complete: {generated_pdf}"
                    )
                    ...
                    await async_sms_dispatch(
                        emp_phone,
                        emp_name
                    )

                    logs.append(
                        f"SMS alert broadcast dispatched to: {emp_phone}"
                    )
                    ...
                    logs.append(
                        f"HTTP Request Node call status returned: {res.get('status_code')}"
                    )
                    ...
                    logs.append(
                        f"Workflow pipeline suspended asynchronously for {delay_seconds}s."
                    )
            ...
            if "condition" not in node_type_normalized:
                ...
                current_node_id = (
                    str(next_edge_generic.get("target"))
                    if next_edge_generic and isinstance(next_edge_generic, dict)
                    else (
                        getattr(next_edge_generic, "target", None)
                        if next_edge_generic
                        else None
                    )
                )

        SagaManager.complete(run_id)
        print(f"[SAGA] Workflow Completed: {run_id}")
        ...
        print(f"[SAGA] Workflow Failed: {e}")
