...
export default function AnalyticsPage() {
  const [range, setRangeState] = useState("all"); const [appliedRange, setAppliedRange] = useState("all"); const [data, setData] = useState(null); const [loading, setLoading] = useState(true); const [error, setError] = useState(null);
  const setRange = (value) => { setRangeState(value); setLoading(true); setError(null); };
  useEffect(() => { let alive = true; axios.get(`${API}/analytics?time_range=${appliedRange}`, { headers: { Authorization: `Bearer ${localStorage.getItem("token")}` } }).then((response) => alive && setData(response.data)).catch((err) => alive && setError(err.response?.data?.detail || "Unable to load analytics data. Please try again.")).finally(() => alive && setLoading(false)); return () => { alive = false; }; }, [appliedRange]);
